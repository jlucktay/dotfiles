...
alias dantfclean='find $HOME/go/src/github.com/Dentsu-Aegis-Network-Global-Technology $HOME/git/github.com/Dentsu-Aegis-Network-Global-Technology \( \( -type d -name ".terraform" \) -o \( -type f -iname "terraform*.*tfstate*" \) \) -exec rm -rfv -- "{}" + | grep -vE "\/[0-9a-f]{32}"'
